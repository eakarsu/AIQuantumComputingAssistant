// AI Quantum algorithm tutor
// Interactive chatbot teaching quantum mechanics and specific algorithms
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIQuantumComputingAssistant'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIQuantumComputingAssistant. Focus area: Quantum algorithm tutor. ${`Interactive chatbot teaching quantum mechanics and specific algorithms`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Quantum algorithm tutor.\n${`Interactive chatbot teaching quantum mechanics and specific algorithms`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'algorithm-tutor', context, parsed);
    res.json({ feature: 'algorithm-tutor', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[algorithm-tutor]', err.message);
